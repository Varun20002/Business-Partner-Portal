-- ═══════════════════════════════════════════════════════════════
-- Seed: DNAP Incentive Program FAQs (11)
-- Notes:
-- - Inserts only if the question doesn't already exist.
-- - Answers can be edited later from the Admin → FAQs screen.
-- - Uses newline formatting so the UI can render multi-line content.
-- ═══════════════════════════════════════════════════════════════
...
Business Partners earn ₹500 for every New Derivatives User (NDU) who completes ₹10 lakh derivatives trading volume in eligible products.',
  0
WHERE NOT EXISTS (SELECT 1 FROM faqs WHERE question = 'What is the DNAP Incentive Program?');
...
• And complete ₹10 lakh derivatives volume',
  1
WHERE NOT EXISTS (SELECT 1 FROM faqs WHERE question = 'Who is a New Derivatives User (NDU)?');
...
• On or before 30 April 2026',
  2
WHERE NOT EXISTS (SELECT 1 FROM faqs WHERE question = 'What volume is required?');
...
• ₹500 per eligible NDU who meets the required derivatives volume.',
  3
WHERE NOT EXISTS (SELECT 1 FROM faqs WHERE question = 'How much incentive will I receive?');
...
• 200 × ₹500 = ₹1,00,000',
  4
WHERE NOT EXISTS (SELECT 1 FROM faqs WHERE question = 'What is the maximum payout?');
...
Your exact baseline number will be shared by the CoinDCX team and is final.',
  5
WHERE NOT EXISTS (SELECT 1 FROM faqs WHERE question = 'What is Baseline?');
...
• Incremental Volume = $15M – $10M = $5M',
  6
WHERE NOT EXISTS (SELECT 1 FROM faqs WHERE question = 'What is Incremental Volume?');
...
• Up to a maximum of ₹500 rebate per user',
  7
WHERE NOT EXISTS (SELECT 1 FROM faqs WHERE question = 'What is the rebate offer?');
...
• ₹500 per user (cap on trading fee rebate).',
  8
WHERE NOT EXISTS (SELECT 1 FROM faqs WHERE question = 'What is the maximum rebate?');
...
• US Stock Futures',
  9
WHERE NOT EXISTS (SELECT 1 FROM faqs WHERE question = 'Does Options volume count for any Incentive?');
...
• Subject to internal validation and reconciliation by CoinDCX.',
  10
WHERE NOT EXISTS (SELECT 1 FROM faqs WHERE question = 'When will payout be credited for all incentives?');
